...
public class Solution3 {
    ...
    public int[] maxSlidingWindow(int[] nums, int k) {
        ...
        for (int i = 0; i < k; i++) {
            // 保持元素值单调递减、索引值单调递减的，因此队尾元素最大。
            // 如果当前元素比队首元素还要大，这个时候如果直接把当前元素添加到队首会违反"单调性"，不能保证值是单调递增的。
            // 由于当前元素比队首还要大，因此队首元素肯定不是滑动窗口内的最大值，因此不断将队首元素出队，直到能够把当前元素塞进去为止。
...
//        System.out.println(deque);
        for (int i = k; i < nums.length ; i++) {
            ...
            deque.offerFirst(i);
            /*else {
                // 要保持单调性，如果队首元素还要小比当前元素还要小，那么队首元素要出队（这么做是安全的，因为队首元素肯定不是滑动窗口内的最大值了）。
    ...
    public static void main(String[] args) {
        ...
        int[] ints = null;
        ints = solution3.maxSlidingWindow(new int[]{1, 3, -1, -3, 5, 3, 6, 7}, 3);
//        // [3, 3, 5, 5, 6, 7]
